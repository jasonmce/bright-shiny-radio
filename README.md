...
2. Update:
   - `main.tf` with your AWS region.
   - `terraform.tfvars` with your Route 53 domain details.
...
4. SSH into the Pi and run the installer:
   ```bash
   bash -c "$(curl -sL https://raw.githubusercontent.com/jasonmce/pi-radio-aws-site/refs/heads/main/pi/install-pi-software.sh)"
   ```
